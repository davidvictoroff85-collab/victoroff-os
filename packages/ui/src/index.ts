export interface TabOptions {
  ...
  panels: HTMLElement[];
  selectedId: string;
  onSelect?: (id: string, userInitiated: boolean) => void;
}

export function activateTabs(options: TabOptions): (id: string, focus?: boolean) => void {
  const select = (id: string, focus = false, userInitiated = false) => {
    ...
    options.onSelect?.(id, userInitiated);
  };

  options.buttons.forEach((button, index) => {
    button.addEventListener("click", () => select(button.dataset.target ?? "", false, true));
    button.addEventListener("keydown", (event) => {
      ...
      if (targetId) select(targetId, true, true);
    });
  });
